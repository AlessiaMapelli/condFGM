...
idx_dlb = c(0, 1, 2, 3, 4, '5_0', 6, 7, 8, 9, 
              ...
              28, 29, '30_1',31 ,'32_2' ,33, 34, 35)

mat_t = c(0,(n-1))
k = 1
for(i in idx_dlb){
  print( paste('Processing patient ', i))
  signal <- npyLoad(file = paste('real_data/dlb_data/array_fd_data/fd_data_', as.character(i), '.npy', sep=''))  ##da modificare x prendere soggetti giusti
...
for(j in 1:p){
  ...
  for (i in idx_dlb){
    print( paste('Processing patient ', i))
    signal <- npyLoad(file = paste('real_data/dlb_data/array_fd_data/fd_data_', as.character(i), '.npy', sep=''))
...
write.csv(fpc.score, "real_data/fpca_scores_dlb.csv")
