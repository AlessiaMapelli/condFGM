...
#########
path_sensors =  "Position_list.Rdata"
...
#################################################
output_path = paste("",config$output_path, sep ="")
name_output = config$name_output
input_path =  paste("", config$input_path, sep ="")
type = config$type
...
for(i in 1:n_groups){
  ...
  adj_df <- melt(G.cov.simm, varnames = c("Row","Col"), value.name = "Value")
  adj_df <- adj_df %>%
    mutate(
      Row = factor(as.numeric(as.character(Row)), levels = 1:64, labels = node.names),
      Col = factor(as.numeric(as.character(Col)), levels = 1:64, labels = node.names)
    )
  
  adj_df <- adj_df %>%
    mutate(Row = factor(Row, levels = rev(node.names)))
  ...
  plot <- ggplot(adj_df, aes(x = Col, y = Row, fill = Value)) +
    ...
    theme(
      axis.text.x = element_text(size = 6, angle = 90, vjust = 0.5, hjust = 1),  # shrink x-axis labels and rotate
      axis.text.y = element_text(size = 6)  # shrink y-axis labels
    )
  print(plot)
