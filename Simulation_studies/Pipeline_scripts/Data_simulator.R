...
source(paste(func.path,"A.prelim.func.red.R", sep="/")) # For Model A diff generation
...
plot <- ggplot(adj_df, aes(x = Col, y = Row, fill = Value)) +
  ...
  scale_y_reverse() +  # to match matrix view
  labs(title = "Score adjacency Matrix", x = "", y = "")
ggsave(plot, filename=paste(foldname, "/Adjacency_matrix_scores_g2.png", sep=""), width=8, height=8, dpi=300)
...
plot <- ggplot(adj_df, aes(x = Col, y = Row, fill = Value)) +
  ...
  scale_y_reverse() +  # to match matrix view
  labs(title = "Node adjacency Matrix", x = "", y = "")
ggsave(plot, filename=paste(foldname, "/Adjacency_matrix_node_g2.png", sep=""), width=8, height=8, dpi=300)
...
save(h,group, file=paste(foldname, "/Original_data_generated.RData", sep=""))

save(G.true.g1, G.true.g2, file=paste(foldname,"/Ground_truth.rda", sep=""))
...
write.csv(fpc.score, paste(foldname, "/fpc_scores.csv", sep=""))
write.csv(data.frame(group=group),paste(foldname, "/grouping_factor.csv", sep=""))
...
save(Scor, file=paste(foldname,"/Score_corr_matrix.rda", sep=""))
...
save(node_Scor, file=paste(foldname,"/Node_corr_matrix.rda", sep=""))
