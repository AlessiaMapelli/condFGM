...
score_path = paste0(save_path, simulation_name,"/", "seed_", iteration, 
                    "/", "fpc_scores_", name_output,".csv")
covariate_path = paste0(save_path, simulation_name,"/", "seed_", iteration, 
                        "/", "covariates_", name_output,".csv")
input_path = paste0(save_path, simulation_name,"/", "seed_", iteration, 
                    "/", "net_est_input_", name_output,".rda")
distance_path = paste0(save_path, simulation_name,"/", "seed_", iteration, 
                       "/", "Node_corr_matrix_", name_output,".rda")
...
if(file.exists(input_path)){
  ...
  fpc.score <- read.csv(score_path)[, -1]
  n_nodes <- ncol(fpc.score)/n_basis
  n_samples <- nrow(fpc.score)
  covariates <- read.csv(covariate_path)[, -1]
  covariates$group <- as.factor(covariates$group)
}
full_data <- cbind(covariates,fpc.score)
if(pre_screen){
